...
class ALBEF(nn.Module):
    def __init__(self,                 
        ...
        if self.distill:
            self.visual_encoder_m = VisionTransformer(
                img_size=config['image_res'], patch_size=16, embed_dim=512, depth=12, num_heads=12, 
                mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6))             
    ...
    def forward(self, image, quesiton, answer=None, alpha=0, k=None, weights=None, train=True):
        ...
        if train:               
            ...
            print(quesiton.input_ids.shape, quesiton.attention_mask.shape, image_embeds.shape, image_atts.shape)
