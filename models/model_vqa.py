...
class ALBEF(nn.Module):
    def __init__(self,                 
        ...
        # configuration = ViTConfig(image_size=config["image_res"], patch_size=16, embed_dim=768, mlp_ratio=4, qkv_bias=True, layer_norm_eps=1e-6)
        ...
        # text_config_encoder = BertConfig.from_json_file(config["bert_config"])   
        # # text_config_encoder.fusion_layer = 6
        # self.text_encoder = BertModel.from_pretrained("bert-base-uncased", config=text_config_encoder, add_pooling_layer=False)
        #transformers-4.14.1
        ...
        if self.distill:
            ...
            # self.text_encoder_m = BertModel.from_pretrained("bert-base-uncased", config=text_config_encoder)
    ...
    def forward(self, image, quesiton, answer=None, alpha=0, k=None, weights=None, train=True):
        ...
        if train:               
            ...
            # print(image_atts.shape, image_embeds.shape, quesiton.input_ids.shape, quesiton.attention_mask.shape)
            ...
            if self.distill:                    
                with torch.no_grad():
                    ...
                    question_output_m = self.text_encoder_m(quesiton.input_ids, 
                                                            attention_mask = quesiton.attention_mask, 
                                                            encoder_hidden_states = image_embeds_m["last_hidden_state"],
                                                            encoder_attention_mask = image_atts,                             
                                                            return_dict = True)    
                    question_states_m = []    
                    # print(question_output_m)            
                    for b, n in enumerate(k):
                ...
                # print(logits_m)
                # print(answer.input_ids.shape, answer.input_ids.shape,  question_states.shape, question_atts.shape,  answer_targets.shape, logits_m["logits"].shape)
